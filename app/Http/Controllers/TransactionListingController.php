   ...
   /**
    ...
    public function generate(Request $request)
    {
        ...
        $sales = DB::table('sales as s')
            ->leftJoin('income_expense_accounts as iea', 's.transaction_type_id', '=', 'iea.transaction_type_id')
            ->leftJoin('sales_details as sd', 'sd.sales_id', '=', 's.id')
            ->leftJoin('sales_invoice_assignment_details as siad', 's.si_assignment_id', '=', 'siad.id')
            ->leftJoin('payments as p', 's.payment_id', '=', 'p.id')
            ->leftJoin('payment_methods as pm', 'p.payment_type', '=', 'pm.name')
            ->leftJoin('item_bundles as ib', 'ib.bundle_name', '=', 'sd.item_code')
            ->groupBy('s.id', 's.invoiced_at', 's.transaction_type_id', 's.branch_id', 'sd.item_name',
                'sd.amount', 'p.payment_type', 'p.details', 'p.created_by', 'siad.prefix_value', 'siad.series_number',
                'iea.income_account', 'pm.code', 'pm.description', 'ib.item_description')
            ->select(
                's.id',
                's.invoiced_at',
...
$unique_item_names = array(); // Associative array to track unique item names
$unique_cell_values = array(); // Associative array to track unique values in cells C to G
$row = 8;
...
foreach ($sales as $ks => $sale) {
    ...
    // Insert rows for data only if the item_name is not in the unique_item_names array
    if (!array_key_exists($sale->item_name, $unique_item_names)) {
        $unique_item_names[$sale->item_name] = true; // Add item_name to the associative array
        $sheet->insertNewRowBefore($row, 1);
        $sheet->setCellValue('B' . $row, $sale->item_name . ' (' . $sale->quantity . ') (' . $sale->ref_no . ')');
        $sheet->getStyle('B' . $row)->getFont()->setBold(true);
        // ... (rest of the data insertion logic)
        $row++;
    }

    // Check if the combination of values in cells C to G is unique
        ...
        $sheet->setCellValue('I' . $row, $sale->item_amount);

        $row++;
        $sheet->setCellValue('C' . $row, $sale->transaction_type_id);
        $sheet->setCellValue('E' . $row, $sale->income_account_code);
        $sheet->setCellValue('F' . $row, $sale->cost_center);
        $sheet->setCellValue('G' . $row, $sale->income_account);

        $sheet->getStyle('J' . $row)->getNumberFormat()->setFormatCode('#,##0.00');
        $sheet->setCellValue('J' . $row, $sale->item_amount);
...
if ($previous_invoiced_at !== null) {
    ...
    // ... (rest of the total calculation logic)
    ...
    public function generateSummary(Request $request)
    {
        ...
        $sales = DB::table('sales')
            ->leftJoin('income_expense_accounts as iea', 'sales.transaction_type_id', '=', 'iea.transaction_type_id')
            ...
            ->leftJoin('payments as p', 'sales.payment_id', '=', 'p.id')
            ->leftJoin('payment_methods as pm', 'p.payment_type', '=', 'pm.name')
            ->leftJoin('item_bundles as ib', 'ib.bundle_name', '=', 'sd.item_code')
            ->select(
                'sales.id',
            ...
            ->groupBy('sales.id', 'sales.invoiced_at', 'sales.transaction_type_id', 'sales.branch_id', 'sd.item_name', 'ib.item_description',
                    'sd.amount', 'p.payment_type', 'p.details', 'p.created_by', 'siad.prefix_value', 'siad.series_number', 'iea.income_account', 'pm.code', 'pm.description')
            ->get();
