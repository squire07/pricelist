    ...
    <div class="container-fluid">
        <div class="row mb-2">
            ...
            <div class="col-sm-6 text-right">
                <button type="button" class="btn btn-info" data-toggle="modal" data-target="#modal-add" {{ Helper::BP(7,2) }}>Add Category</button>
            </div>
        </div>
    </div>
@stop
...
@section('content')
    <div class="container-fluid">
        <div class="card">
            <div class="card-body table-responsive" style="overflow:auto;width:100%;position:relative;">
                <table id="dt_category" class="table table-bordered table-hover table-striped" width="100%">
                    <thead>
                        <tr>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <button type="button" class="btn btn-sm btn-default btn_show" 
                                        ...
                                        data-category-updated_by="{{ $category->updated_by }}"
                                        {{ $show_button_state }}>
                                    ...
                                    <button type="button" class="btn btn-sm btn-primary btn_edit" 
                                        ...
                                        data-uuid="{{ $category->uuid }}" 
                                        data-category-name="{{ $category->name }}" 
                                        data-category-remarks="{{ $category->remarks }}"
                                        {{ $edit_button_state }}>
    ...
    <div class="modal fade" id="modal-add" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="form-horizontal" action="{{ route('customer-categories.store') }}" method="POST" id="form_modal_add" autocomplete="off">
                    @csrf
                    <div class="modal-body">
                        <div class="container-fluid">
                            <div class="col-12" data-required="true">
    ...
    <div class="modal fade" id="modal-show" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Customer Category Details</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.btn_edit', function() {
            ...
            var remarks = $(this).attr("data-category-remarks");

            $('#modal_edit_name').val(name); 
            $('#modal_show_remarks').val(remarks);

            // define the edit form action
            let action = window.location.origin + "/customer-categories/" + uuid;
        ...
        $(document).on('click', '.btn_show', function() {
            ...
            var name = $(this).attr("data-category-name");
            var remarks = $(this).attr("data-category-remarks");
            var updated_by = $(this).attr("data-category-updated_by");
