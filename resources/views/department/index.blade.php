    ...
    <div class="container-fluid">
        <div class="row mb-2">
            ...
            <div class="col-sm-6 text-right">
                <button type="button" class="btn btn-info" data-toggle="modal" data-target="#modal-add" {{ Helper::BP(13,2) }}>Add Department</button>
            </div>
        </div>
    </div>
@stop
...
@php
    ...
    $edit_button_state = Helper::BP(13,4);
@endphp
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-body table-responsive" style="overflow:auto;width:100%;position:relative;">
                <table id="dt_department" class="table table-bordered table-hover table-striped" width="100%">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        @foreach($departments as $department)
                            <tr>
                                ...
                                <td class="text-center">{{ $department->created_by }}</td>
                                <td class="text-center">{{ $department->created_at }}</td>
                                <td class="text-center">{{ $department->updated_by }}</td>
                                ...
                                <td class="text-center">
                                    <button type="button" class="btn btn-sm btn-default btn_show" 
                                        ...
                                        data-uuid="{{ $department->uuid }}"
                                        data-department-name="{{ $department->name }}" 
                                        data-department-updated_by="{{ $department->updated_by }}"
                                        {{ $show_button_state }}>
                                    ...
                                    <button type="button" class="btn btn-sm btn-primary btn_edit" 
                                        ...
                                        data-uuid="{{ $department->uuid }}" 
                                        data-department-name="{{ $department->name }}" 
                                        data-department-remarks="{{ $department->remarks }}"
                                        {{ $edit_button_state }}>
    ...
    <div class="modal fade" id="modal-add" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="form-horizontal" action="{{ route('departments.store') }}" method="POST" id="form_modal_add" autocomplete="off">
                    @csrf
    ...
    $(document).ready(function() {
        $('#dt_department').DataTable({
            ...
            initComplete: function () {
                $("#dt_department").wrap("<div style='overflow:auto;width:100%;position:relative;'></div>");
        ...
        $(document).on('click', '.btn_edit', function() {
            ...
            var remarks = $(this).attr("data-department-remarks");

            $('#modal_edit_name').val(name); 
            $('#modal_show_remarks').val(remarks);

            // define the edit form action
            let action = window.location.origin + "/departments/" + uuid;
        ...
        $(document).on('click', '.btn_show', function() {
            ...
            var name = $(this).attr("data-department-name");
            var remarks = $(this).attr("data-department-remarks");
            var updated_by = $(this).attr("data-department-updated_by");
