                ...
                <div class="card-body">
                    <table class="table table-borderless" style="width:100%">
                        <tbody style="width:inherit">
                            <tr>
                                <td></td>
                                <td class="text-right text-bold" style="width:30%">Sub Total</td>
                                <td class="text-right" style="width:35%">{{ number_format($delivery->total_amount, 2) }}</td>
                            </tr>
                            <tr>
                                <td></td>
                                <td class="text-right text-bold" style="width:30%">Less Discount</td>
                                <td class="text-right" style="width:35%">{{ number_format($delivery->add_discount_value, 2) }}</td>
                            </tr>
                            <tr>
                                <td></td>
                                <td class="text-right text-bold" style="width:30%">Net Subtotal</td>
                                <td class="text-right" style="width:35%">{{ number_format($delivery->total_amount, 2) }}</td>
                            </tr>
                            <tr>
                                <td></td>
                                <td class="text-right text-bold" style="width:30%">Tax</td>
                                <td class="text-right" style="width:35%">{{ number_format($delivery->vat_amount, 2) }}</td>
                            </tr>
                            <tr>
                                <td></td>
                                <td class="text-right text-bold" style="width:30%">Grand Total</td>
                                <td class="text-right text-bold" style="width:35%">{{ number_format($delivery->grandtotal_amount, 2) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="modal fade" id="modal-submit-payment" data-backdrop='static'>
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form class="form-horizontal" action="{{ route('delivery.update', $delivery->uuid)}}" method="POST" id="form_modal_submit_payment" autocomplete="off">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="modal-body form-horizontal">
                        ...
                        <div class="col-12">
                            <div class="form-group row">
                                ...
                                <div class="col-sm-4">
                                    <input type="text" class="form-control form-control-sm text-right" id="total_amount" value="{{ number_format($delivery->grandtotal_amount, 2) }}" readonly tabindex="-1">
                                </div>
                            </div>
                        </div>
...
$(document).ready(function() {
    ...
    $('#btn_modal_print_invoice').on('click', function() {
        ...
        let balanceValue = parseFloat($('#balance').val());  // Assuming #balance is an input field

        if (balanceValue !== 0) {
            // Show Swal alert for confirmation
            Swal.fire({
                title: 'Insufficient Payment!',
                text: 'Proceed for partial payment?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    // Check if all amount fields are valid and not zero
                    let allAmountsValid = true;

                    $('.input_amount_field').each(function(index) {
                        let amountFieldValue = parseFloat($(this).val());

                        if (isNaN(amountFieldValue) || amountFieldValue === 0) {
                            allAmountsValid = false;
                            return false; // Exit the loop early if any amount is invalid
                        }
                    });
                    ...
                    if (!allAmountsValid) {
                        Swal.fire({
                            ...
                            text: "Please ensure all payment amounts are valid and not zero.",
                            icon: "error",
                        });
                        return; // Prevent further action if amounts are invalid
                    }

                    // User confirmed and all amounts are valid, proceed with form submission
                    $('#form_modal_submit_payment').submit();
                }
            });
        } else {
            // Balance is 0, no need for confirmation, proceed directly
            $('#form_modal_submit_payment').submit();
        }
    });
